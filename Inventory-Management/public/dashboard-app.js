...
const navigationConfig = {
    admin: [
        ...
        { id: 'restock', icon: '🔄', label: 'Restock', page: 'renderRestockPage' },
        { id: 'staff-reports', icon: '👔', label: 'Staff Reports', page: 'renderStaffReports' },
        { id: 'audit-logs', icon: '📋', label: 'Audit Logs', page: 'renderAuditLogs' },
        { id: 'notifications', icon: '🔔', label: 'Notifications', page: 'renderNotifications' },
        { id: 'email-recipients', icon: '📧', label: 'Email Recipients', page: 'renderEmailRecipients' },
        ...
        return `
            <tr>
                ...
                <td class="table-actions">
                    <button class="btn btn-sm btn-secondary edit-user" data-id="${user._id}">✏️ Edit</button>
...
function loadProducts(list = items) {
    ...
    tbody.innerHTML = list.map(item => {
        ...
        if (canEdit) {
            actionButtons.push(`<button class="btn btn-sm btn-secondary edit-product" data-id="${item._id}">✏️ Edit</button>`);
        }
...
function loadSuppliers() {
    ...
    tbody.innerHTML = data.map(supplier => {
        ...
        const actionButtons = [];
        if (hasId && canManage) {
            actionButtons.push(`<button class="btn btn-sm btn-secondary edit-supplier" data-id="${supplier._id}">✏️ Edit</button>`);
        }
        if (hasId && canDelete) {
